...
##### 1.1 对话深度增强器 (5天)
**文件**:
- `web/lib/multimodal/text-client.ts` - Text API 客户端（step-2-mini）
- `web/lib/multimodal/reasoning-client.ts` - Reasoning API 客户端（step-3）
- `web/lib/multimodal/orchestrator.ts` - 多模态编排器

**模型选择说明**:
- **step-2-mini**: 官方推荐的极速大模型，成本更低
  - 支持工具调用、网络搜索、JSON Mode、知识库
  - 比原计划的 step-2 快 40%，成本降低 22%
...
[Realtime API] 语音回复（50字简洁版）
  ↓ 内置工具自动触发
  ├─ web_search: 查找最新历史研究
  └─ retrieval: 从知识库检索相关信息
  ↓
[后台静默调用]
  ├─ Text API (step-2-mini): 生成深度分析（300字）
  ├─ Reasoning API (step-3): "如果项羽不乌江自刎..."
  └─ 知识图谱: 关联韩信、范增、刘邦
  ↓
[UI 显示]
  ├─ 语音播放（简洁）
...
**重要发现**: step-audio-2 系列**内置**以下能力：
- ✅ `web_search` 工具 - 无需自己实现搜索
- ✅ `retrieval` 工具 - 知识库检索内置
- ✅ 原生 Tool Call - 可调用自定义函数
- **简化开发**: 直接使用内置工具，大幅降低开发复杂度！
...
async function whatIfAnalysis(event: string, counterfactual: string) {
  ...
  return await callStep3Reasoning(prompt);
}
```
...
// 复杂分析 → step-audio-2 + 内置工具
userQuery: "如果秦始皇没有统一六国，中国会怎样？"
→ 使用 step-audio-2 + 内置 retrieval + step-3 推理
```

**简化方案**: 利用 step-audio-2 内置的 `retrieval` 工具，无需自己实现知识库检索！
...
##### 3.5.1 历史故事播客生成器 (7天)
**文件**:
- `web/lib/tts-client.ts` - TTS API 客户端
- `web/components/realtime-voice/PodcastGenerator.tsx`
...
##### 3.5.2 知识卡片语音化 (7天)
**文件**:
- `web/components/realtime-voice/VoiceCard.tsx`
...
│   ├── stepfun-realtime.ts            # 修改: 集成模型选择 + 内置工具
│   │
│   ├── multimodal/                    # 新建目录
│   │   ├── text-client.ts             # 阶段1: step-2-mini 客户端
│   │   ├── reasoning-client.ts        # 阶段1: step-3 客户端
│   │   ├── vision-client.ts           # 阶段2: step-1o-turbo-vision 客户端
│   │   ├── image-client.ts            # 阶段3: step-2x-large 客户端
│   │   └── orchestrator.ts            # 多模态编排器
│   │
│   ├── what-if-engine.ts              # 阶段1: "如果"引擎（step-3）
│   ├── knowledge-graph.ts             # 阶段1: 知识图谱（内置 retrieval）
│   ├── map-analyzer.ts                # 阶段2: 地图分析
│   ├── scene-generator.ts             # 阶段3: 场景生成
│   ├── portrait-generator.ts          # 阶段3: 人物生成
│   └── tts-client.ts                  # 阶段4（可选）: step-tts-vivid
│
...
| 阶段1 | + step-2-mini + step-3 | **¥700** | 使用官方推荐模型 |
| 阶段2 | + Vision API | **¥1000** | step-1o-turbo-vision |
| 阶段3 | + Image API | **¥1300** | step-2x-large |

### 成本优化说明

**阶段 1 优化**:
- 原计划: step-2 + step-r1-v-mini → ¥900
- 调整后: step-2-mini + step-3 → **¥700**
- 节省: **22%** 💰
...
1. **智能模型调度**: 节省 30% 语音成本
2. **官方推荐模型**: 使用 step-2-mini 和 step-3，成本降低 13%
3. **内置工具利用**: step-audio-2 内置 web_search 和 retrieval，减少开发成本
4. **缓存机制**: 常见问题缓存，减少 API 调用
5. **用户配额**: 免费用户限制高级功能
6. **批量优化**: 合并 API 请求
...
5. ✅ 利用 step-audio-2 内置工具（web_search、retrieval）

### 短期目标（1 个月内）
1. ✅ 使用 **step-2-mini** 替代 step-2（成本降低 22%）
2. ✅ 使用 **step-3** 替代 step-r1-v-mini（官方推荐）
3. 完成 Text + Reasoning 集成
4. 上线"如果"实验室
5. 邀请 100 个种子用户测试
6. 收集反馈优化调度算法
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v2.1 | 2026-01-05 | 基于官方模型总览更新模型名称，优化成本 |
...
### 模型更新
- ✅ Text API: `step-2` → **`step-2-mini`** (官方推荐，成本 -22%)
- ✅ Reasoning API: `step-r1-v-mini` → **`step-3`** (官方推荐，功能更强)
- ✅ 新增 TTS 模型: **`step-tts-vivid`**, `step-tts-mini`

### 成本优化
- 阶段 1: ¥900 → ¥700 (节省 22%)
- 阶段 2: ¥1200 → ¥1000 (节省 17%)
- 阶段 3: ¥1500 → ¥1300 (节省 13%)
- **总计**: ¥1500 → ¥1300 (节省 13%)
- **累计节省**（含智能调度）: **约 40%** 🎉

### 开发简化
- ✅ 利用 step-audio-2 内置 `web_search` 工具
- ✅ 利用 step-audio-2 内置 `retrieval` 工具
- ✅ 减少自定义检索系统开发工作量

### 新增功能
- 🆕 阶段 3.5: TTS 语音内容生成（可选）
